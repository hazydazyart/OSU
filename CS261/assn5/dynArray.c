// Assignment 5 - due 5/25/2014
// Megan Conley / conleyme@onid.oregonstate.edu
// CS261-400
// dynArray implementation file
...
int _smallerIndexHeap(DynArr *heap, int i, int j)
{
  ...
  int cmp = compare(getDynArr(heap, i), getDynArr(heap, j));
  if (cmp == 1) {
	return j;
  } else {
	return i;
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
   ...
   if(rightIdx < max) {
	...
	if(compare(getDynArr(heap, pos), getDynArr(heap, minIdx)) == -1) {
