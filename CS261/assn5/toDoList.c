...
{
	Task* newTask = (Task*)malloc(sizeof(Task)); // sizeof(Task*) is original
	assert(newTask != 0);
	newTask->priority = priority;
	strcpy(newTask->description, desc);
	printf("Creating priority %d Desc: %s\n", newTask->priority, newTask->description);
	return newTask;
...
void printList(DynArr *heap)
{
	...
	Task* task = (Task*)getDynArr(heap, i);
	printf("%d\t%s\n", task->priority, task->description);
    }
}
