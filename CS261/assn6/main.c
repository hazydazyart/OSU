...
// Assignment 6 - hash map implementation of a concordance
...
int main (int argc, const char * argv[]) {
	...
	while(word != NULL) {
		if(containsKey(hashTable, word) != 0) { // if it already exists
			value = atMap(hashTable, word); // find its value
			value++; // add one
			insertMap(hashTable, word, value); // update value in map
		} else {
			insertMap(hashTable, word, 1); // otherwise add to map as new entry
		}
		word = getWord(fileptr);
	}	
