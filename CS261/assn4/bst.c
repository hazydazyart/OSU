...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	if(cur == 0) {
		...
		if (cur->right != 0 && cur->left != 0) {
			// Value found; has two children
			temp = _leftMost(cur->right);
			cur->val = temp->val;
			cur->right = _removeNode(cur->right, temp->val);
		} else {
			// Value found; has one child
			temp = cur;
			if(cur->left == 0) {
				cur = cur->right;
			} else if (cur->right == 0) {
