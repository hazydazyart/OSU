...
// 4/27/2014
// Assignment 3 - circular deque implementation
...
{
	struct DLink * iter = (struct DLink *)malloc(sizeof(struct DLink*));
...
int isEmptyCirListDeque(struct cirListDeque *q) {
  	if(q->Sentinel->next == 0 && q->Sentinel->prev == 0) { 
		return 1;
	} else {
		return 0;
	}
...
void reverseCirListDeque(struct cirListDeque *q)
{
	...
	struct DLink *current = temp->next; // Start the loop at first link
	temp->next = 0;
	temp->prev = current;
	while(current != q->Sentinel) { // Iterate through until it reaches the sentinel
		current->prev = current->next; // Swap prev/next values and continue
		current->next = temp;
		temp = current;
		current = current->prev;
	}
	q->Sentinel = temp; // Set sentinel to have swapped links
}
